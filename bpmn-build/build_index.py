...
# -*- coding: utf-8 -*-
"""
Build leren-hub/bpmn-cmmn-dmn/index.html.
...
from build import TITLES, NIVEAUS, pad, ROOT, OUT
...
def main():
    ...
    note = (f'<div class="note teal"><b>Alle 30 delen zijn omgezet.</b> Elk deel heeft een sticky '
            f'inhoudsopgave, per-sectie leesvoortgang (lokaal in uw browser bewaard) en een apart '
            f'werkboek-onderdeel met de opdrachten letterlijk uit het werkboek. De downloadbare '
            f'.docx/.pptx-bestanden per deel blijven bereikbaar via de oude vormgeving.</div>')

    intro = (f'<p>Deze Ductus-opleiding behandelt de drie samenhangende OMG-notatiestandaarden voor '
              f'procesmodellering, casemanagement en beslislogica in drie niveaus: het fundament '
              f'(procesdenken, de descriptive en analytic subclass van BPMN, de basis van CMMN en DMN, '
              f'en wanneer je welke taal kiest), de praktijklaag (procesarchitectuur, gevorderd '
              f'modelleren in alle drie de talen, procesmining en governance) en het meesterschapsniveau '
              f'(enterprise-procesarchitectuur, compliance en risicobeheersing in modellen, '
              f'beslisautomatisering op schaal, en het leiden van een transformatie). Eén doorlopende '
              f'casus bij pensioenuitvoerder Meridiaan Pensioen loopt door alle drie niveaus.</p>')

    rows = []
    for lv in NIVEAUS:
        rows.append(
            f'<div class="rowhead"><h3>Niveau {lv["n"]} — {lv["label"]}</h3>'
            f'<span>{esc(lv["cert"])} · deel {lv["lo"]}–{lv["hi"]}</span><div class="line"></div></div>\n'
...
<title>BPMN, CMMN &amp; DMN | Eductus</title>
<meta name="description" content="Cursus BPMN, CMMN &amp; DMN in de warme, redactionele vormgeving van de SAP-leerreeks — 30 delen, procesmodellering, casemanagement en beslislogica, OCEB 2 Fundamental → Business Intermediate + BPM+ → Business Advanced + ABPMP CBPP.">
<link rel="canonical" href="https://leren.eductus.nl/bpmn-cmmn-dmn/index.html">
...
<div class="wrap">
  <div class="top">
    <div class="topbar">
      ...
      <nav class="sitenav" aria-label="Eductus-onderdelen">
        <a href="https://cursus.eductus.nl/bpmn-cmmn-dmn/index.html">↔ Oude vormgeving (cursus.eductus.nl)</a>
  ...
  <section class="hero">
    <p class="eyebrow">BPMN, CMMN &amp; DMN</p>
    <h1>30 delen,<br><em>nieuwe vormgeving</em>.</h1>
    <p class="hero__lead">Procesmodellering (BPMN), casemanagement (CMMN) en beslislogica (DMN) in samenhang, nu in de warme, redactionele opmaak van de SAP-leerreeks: secties met leesvoortgang, en een apart werkboek-onderdeel per deel met de opdrachten letterlijk uit het werkboek. Doorlopende casus bij pensioenuitvoerder Meridiaan Pensioen. Dit is nu de vaste vorm op leren.eductus.nl; de oude documentstijl blijft bereikbaar op cursus.eductus.nl.</p>
  </section>
  ...
  <footer>Eductus · BPMN, CMMN &amp; DMN · 3 niveaus · 30 delen</footer>
</div>
...
(function(){{
  ...
  function deelCard(n){{
    ...
    try{{
      var state=JSON.parse(localStorage.getItem("eductus_bcd"+n+"_progress_v1")||"{{}}");
    ...
    a.innerHTML=
      ...
      '<p>'+total+' secties, leesvoortgang, en de werkboekopdrachten van dit deel.</p>'+
